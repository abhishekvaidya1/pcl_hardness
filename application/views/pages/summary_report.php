                                        ...
                                        <div class="panel-body">
                                            <table border="1" align="center" class="customers2" style="width: 100%;">
                                                ...
                                                <tr >
                                                    ...
                                                    <td style="text-align: center; font-size: 17px; background-color:lightgrey; vertical-align: middle;" rowspan="2">Option to view<br/>the reports</td>
                                                    <td style="text-align: center; font-size: 17px; background-color:lightgrey; vertical-align: middle;" rowspan="2">Option to submit<br/>the reports</td>
                                                </tr>
                                                <tr >                                           
                                                    ...
                                                    <?php foreach ($data as $row) { ?>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <?php if ($arr1[$row->heat_no.$row->c_date.$row->item]['printed'] == 'no') { ?>
                                                                ...
                                                                if ($row->l_status == "0") {
                                                                    ?>
                                                            <td rowspan="<?php echo $arr1[$row->heat_no.$row->c_date.$row->item]['rowspan']; ?>"  style="text-align: center; background-color: lightgoldenrodyellow; font-size: 17px; vertical-align: middle;" class="statusupdate">&nbsp;&nbsp;Pending&nbsp;&nbsp;</td>
    <?php } else { ?><td rowspan="<?php echo $arr1[$row->heat_no.$row->c_date.$row->item]['rowspan']; ?>"  style="text-align: center; background-color: lightgreen; font-size: 17px; vertical-align: middle;" class="statusupdate">&nbsp;&nbsp;Submitted&nbsp;&nbsp;</td><?php } ?>
    <td rowspan="<?php echo $arr1[$row->heat_no.$row->c_date.$row->item]['rowspan']; ?>"  style="text-align: center; vertical-align: middle;"><button class="btn btn-primary pull-center view_report" type="button" value="<?php echo $row->heat_no . ',' . $row->item . ',' . $row->c_date; ?>" style="margin: 3px;">View</button></td>
    <td rowspan="<?php echo $arr1[$row->heat_no.$row->c_date.$row->item]['rowspan']; ?>"  style="text-align: center; vertical-align: middle;" class="closestd"><button class="btn btn-primary pull-center submit_report" type="button" value="<?php echo $row->heat_no . ',' . $row->item . ',' . $row->c_date; ?>" style="margin: 3px;" <?php if($row->l_status == "1"){ ?>disabled<?php } ?>>Submit</button></td>
...
var this_val=$(this).val();
   $.ajax({
type: "post",
url: "<?php echo base_url('login_controller/submit_item'); ?>",
data: {submit_data:this_val},
cache: false,
async: false,
success: function (data) {
swal({
...
//$('.closestd').closest('td.statusupdate').css('background-color', '#cd0000');
});
</script>
</body>